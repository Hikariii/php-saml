...
class Utils
{
    ...
    public static function redirect($url, $parameters = array(), $stay = false)
    {
        ...
        if (!preg_match('@^https?:\/\/@i', $url)) {
            throw new Exception\Error(
                ...
                Exception\Error::REDIRECT_INVALID_URL
            );
        }
     ...
     *
     * @return \DOMNode  The decrypted element.
     *
    ...
    public static function decryptElement(\DOMElement $encryptedData, XMLSecurityKey $inputKey)
    {
        ...
        if (!$symmetricKey) {
            throw new Exception\ValidationError(
                ...
                Exception\ValidationError::KEY_ALGORITHM_ERROR
            );
        }
        ...
        if (!$symmetricKeyInfo) {
            throw new Exception\ValidationError(
                ...
                Exception\ValidationError::KEYINFO_NOT_FOUND_IN_ENCRYPTED_DATA
            );
        }
        ...
        if (!$newDoc) {
            throw new Exception\ValidationError(
                ...
                Exception\ValidationError::INVALID_XML_FORMAT
            );
        }
        ...
        if ($decryptedElement === null) {
            throw new Exception\ValidationError(
                ...
                Exception\ValidationError::MISSING_ENCRYPTED_ELEMENT
            );
        }
    ...
    public static function validateBinarySign($messageType, $getData, $idpData, $retrieveParametersFromServer = false)
    {
        ...
        if ((!isset($idpData['x509cert']) || empty($idpData['x509cert'])) && !$existsMultiX509Sign) {
            throw new Exception\Error(
                ...
                Exception\Error::CERT_NOT_FOUND
            );
        }
            ...
            if ($signAlg != XMLSecurityKey::RSA_SHA1) {
                try {
                    ...
                    $ex = new Exception\ValidationError(
                        ...
                        Exception\ValidationError::INVALID_SIGNATURE
                    );
