...
class Settings
{
     ...
     * @param boolean $spValidationOnly Only validate SP data
     *
     * @throws Exception\Error If any settings parameter is invalid
     */
    public function __construct(array $settings, $spValidationOnly = false)
    {
        ...
        if (!is_array($settings)) {
            throw new Exception\Error(
                ...
                Exception\Error::SETTINGS_INVALID,
    ...
    {
        $metadata = Metadata\Metadata::builder($this->_sp, $this->_security['authnRequestsSigned'], $this->_security['wantAssertionsSigned'], null, null, $this->getContacts(), $this->getOrganization());

        $certNew = $this->getSPcertNew();
        if (!empty($certNew)) {
            $metadata = Metadata\Metadata::addX509KeyDescriptors(
                $metadata,
                $certNew,
        ...
        if (isset($this->_security['signMetadata']) && $this->_security['signMetadata'] !== false) {
            if ($this->_security['signMetadata'] === true) {
                ...
                if (!$keyMetadata) {
                    throw new Exception\Error(
                        ...
                        Exception\Error::PRIVATE_KEY_FILE_NOT_FOUND
                    );
                }
                ...
                if (!$certMetadata) {
                    throw new Exception\Error(
                        ...
                        Exception\Error::PUBLIC_CERT_FILE_NOT_FOUND
                    );
                }
            } else {
                if (!isset($this->_security['signMetadata']['keyFileName'])
                    ...
                    throw new Exception\Error(
                        ...
                        Exception\Error::SETTINGS_INVALID_SYNTAX
                    );
                }
            ...
            $metadata = Metadata\Metadata::signMetadata($metadata, $keyMetadata, $certMetadata, $signatureAlgorithm, $digestAlgorithm);
        }
        return $metadata;
    }
    ...
    /**
     * Returns the base url set on the settings if any.
     *
